...
// Mock the ai module's helper functions
vi.mock("ai", async () => {
  ...
  return {
    ...
    getToolOrDynamicToolName: (part: any) => {
...
function makeMessage(overrides: Partial<UIMessage> & { id: string; role: "user" | "assistant" }): UIMessage {
  return {
    parts: [],
    ...overrides,
  } as UIMessage;
...
test("MessageList handles empty parts array with loading", () => {
  const messages: UIMessage[] = [
    makeMessage({
      id: "1",
      role: "assistant",
